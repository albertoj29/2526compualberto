...
int realizarPasoMonteCarlo(int s[N][N], double T) { // ¡OJO! Cambiado de 'void' a 'int'

    //#//CUENTO CUANTOS ESPINES SE DAN LA VUELTA EN ESTE PASO DE MONTE CARLO
    int cambios = 0; // Nuestro contador para saber cuántos espines se dan la vuelta

    //#//BUCLE HASTA N*N PARA INTENTAR CAMBIAR TODOS LOS ESPINES DE LA RED
    for (int n = 0; n < N * N; n++) {
        ...
        if (dist01(gen) < p) {
            ...
            //#//ANOTAMOS QUE HA HABIDO UN CAMBIO DE ESPÍN
            cambios++; 
        }
    ...
    //#//MUESTRO CUANTOS ESPINES HAN CAMBIADO EN ESTE PASO DE MONTE CARLO
    return cambios; // Devolvemos cuántos espines han cambiado en total en este paso
}
    ...
    */
    ...
    for (int t = 0; t < 5; t++) {
        ...
        for (int paso = 0; paso < k_animacion; paso++) {
            // ---> AQUI ESTA EL CAMBIO: Guardamos el numero de cambios
            int cambios_realizados = realizarPasoMonteCarlo(s, T_actual);

            //#// SOLO GUARDAMOS EL ESTADO DE LA RED SI HA HABIDO CAMBIOS DE ESPÍN EN ESTE PASO DE MONTE CARLO
    ...
    cout << "Los 5 archivos de animacion han sido creados." << endl;
    ...
    for (double T = 1.0; T <= 4.0; T += 0.2) {
        ...
        for (int p_mag = 0; p_mag < k_mag; p_mag++) {
            // Aquí no hace falta guardar los cambios porque en esta fase solo queremos que el sistema evolucione
